...
const APP_NAME = process.env.ELASTICSEARCH_NAME ?? 'simple-jobs-node-fastify-with-mdc';
...
function logJob(jobName) {
  ...
  const msg = `ES+MDC : Batch scheduler job ${fields.jobExecutionId} tracked by node fastify with MDC [${runId}]`;
  console.log(msg);
...
function riskyJob() {
  ...
  const emit = () => {
    ...
    const msg = `ES+MDC : Risky job produced ${level} API latency sample responseTime=${responseTime}ms percentile=${fields.percentileTarget} slowQueryAlarm=${fields.slowQueryAlarm} by node fastify with MDC [${runId}]`;
...
function slowJob() {
  ...
  setTimeout(() => {
    ...
    const msg = `ES+MDC : Slow job method responseTime=${responseTime}ms slowMethodAlarm=${fields.slowMethodAlarm} by node fastify with MDC [${runId}]`;
