...
appender = ElasticsearchJobAppender(
    ...
    app=os.getenv('ELASTICSEARCH_NAME', 'simple-jobs-python-flask'),
...
def do_system_job():
    msg = f'ES : Just do system job by python flask [{uuid.uuid4()}]'
    logger.info(msg)
...
@with_retry(max_attempts=3, backoff=1.0)
def do_manager_job():
    msg = f'ES : Just do manager job by python flask [{uuid.uuid4()}]'
    logger.info(msg)
...
@with_retry(max_attempts=3, backoff=1.0)
def do_operator_job():
    msg = f'ES : Just do operator job by python flask [{uuid.uuid4()}]'
    logger.info(msg)
...
def do_risky_job():
    ...
    if random.random() < 0.8:
        msg = f'ES : Risky job completed normally by python flask [{run_id}]'
        logger.info(msg)
    ...
    msg = f'ES : Risky job found unstable condition by python flask [{run_id}]'
    if level == 'WARN':
        logger.warning(msg)
    else:
        logger.error(msg)
