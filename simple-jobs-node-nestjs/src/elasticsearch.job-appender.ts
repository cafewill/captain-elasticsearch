...
/**
 * Elasticsearch _bulk API 어펜더 — Node.js 내장 모듈만 사용 (추가 의존성 없음)
 *
 * 다른 프로젝트 반영 시 이 파일 하나만 복사
 *   1. src/ 에 복사 (패키지명 불필요, 클래스 그대로 사용)
 *   2. .env 에 ELASTICSEARCH_* 환경변수 추가
 *   3. import { ElasticsearchJobAppender } from './elasticsearch.job-appender' 후 인스턴스 생성
 */

export interface ElasticsearchJobAppenderConfig {
  url?:                  string;
  username?:             string;
  password?:             string;
  app?:                  string;
...
export class ElasticsearchJobAppender {
  ...
  }: ElasticsearchJobAppenderConfig = {}) {
    this.index      = `logs-${app}`;
    this.app        = app;
    this.env        = env;
