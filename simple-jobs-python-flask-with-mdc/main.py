...
appender = ElasticsearchJobAppender(
    ...
    app=os.getenv('ELASTICSEARCH_NAME', 'simple-jobs-python-flask-with-mdc'),
...
def log_job(job_name):
    ...
    msg = f'ES+MDC : Batch scheduler job {fields["jobExecutionId"]} tracked by python flask with MDC [{run_id}]'
    logger.info(msg)
...
def do_risky_job():
    ...
    msg = f'ES+MDC : Risky job produced {level} API latency sample responseTime={response_time}ms percentile={fields["percentileTarget"]} slowQueryAlarm={fields["slowQueryAlarm"]} by python flask with MDC [{run_id}]'
...
def do_slow_job():
    ...
    msg = f'ES+MDC : Slow job method responseTime={response_time}ms slowMethodAlarm={fields["slowMethodAlarm"]} by python flask with MDC [{run_id}]'
