...
import { ElasticsearchJobAppender } from './elasticsearch.job-appender';
...
export class ScheduleService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly appender: ElasticsearchJobAppender;
  ...
  constructor(private readonly schedulerRegistry: SchedulerRegistry) {
    this.appender = new ElasticsearchJobAppender({
      url: process.env.ELASTICSEARCH_URL ?? 'http://localhost:9200',
      username: process.env.ELASTICSEARCH_USERNAME ?? '',
      password: process.env.ELASTICSEARCH_PASSWORD ?? '',
      app: process.env.ELASTICSEARCH_NAME ?? 'simple-jobs-node-nestjs-with-mdc',
  ...
  private logJob(jobName: string): void {
    ...
    const msg = `ES+MDC : Batch scheduler job ${fields.jobExecutionId} tracked by node nestjs with MDC [${runId}]`;
    this.logger.log(msg);
  ...
  private riskyJob(): void {
    ...
    const emit = () => {
      ...
      const msg = `ES+MDC : Risky job produced ${level} API latency sample responseTime=${responseTime}ms percentile=${fields.percentileTarget} slowQueryAlarm=${fields.slowQueryAlarm} by node nestjs with MDC [${runId}]`;
      if (level === 'ERROR') {
  ...
  private slowJob(): void {
    ...
    setTimeout(() => {
      ...
      const msg = `ES+MDC : Slow job method responseTime=${responseTime}ms slowMethodAlarm=${fields.slowMethodAlarm} by node nestjs with MDC [${runId}]`;
      if (slowMethod) {
        this.logger.warn(msg);
      } else {
        this.logger.log(msg);
