...
const appender = new ElasticsearchJobAppender({
  ...
  app:                  process.env.ELASTICSEARCH_NAME                ?? 'simple-jobs-node-fastify',
...
setInterval(() => withRetry(() => {
  const msg = `ES : Just do system job by node fastify [${randomUUID()}]`;
  console.log(msg);
  appender.log('INFO', msg, { job: 'system-job' });
}), SYSTEM_DELAY);

setInterval(() => withRetry(() => {
  const msg = `ES : Just do manager job by node fastify [${randomUUID()}]`;
  console.log(msg);
  appender.log('INFO', msg, { job: 'manager-job' });
}), MANAGER_DELAY);

setInterval(() => withRetry(() => {
  const msg = `ES : Just do operator job by node fastify [${randomUUID()}]`;
  console.log(msg);
...
setInterval(() => {
  ...
  if (Math.random() < 0.8) {
    const msg = `ES : Risky job completed normally by node fastify [${runId}]`;
    console.log(msg);
  ...
  setTimeout(() => {
    ...
    const msg = `ES : Risky job found unstable condition by node fastify [${runId}]`;
