...
public class ScheduleService {
	...
	public void doSystemJob() {
		String message = String.format("ES : Just do system job by spring maven [%s]", UUID.randomUUID());
		log.info(message);
	}
	...
	public void doManagerJob() {
		String message = String.format("ES : Just do manager job by spring maven [%s]", UUID.randomUUID());
		log.info(message);
	}
	...
	public void doOperatorJob() {
		String message = String.format("ES : Just do operator job by spring maven [%s]", UUID.randomUUID());
		log.info(message);
	}
	...
	public void doRiskyJob() throws InterruptedException {
		...
		if (chance < 80) {
			log.info("ES : Risky job completed normally by spring maven [{}]", runId);
			return;
		}

		Thread.sleep(ThreadLocalRandom.current().nextLong(3000, 10001));
		String message = String.format("ES : Risky job found unstable condition by spring maven [%s]", runId);
