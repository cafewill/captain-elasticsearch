...
export interface ElasticsearchWebAppenderConfig {
  url?:                  string;
  username?:             string;
  password?:             string;
  app?:                  string;
...
@Injectable()
export class ElasticsearchWebAppender implements NestMiddleware {
  ...
  constructor() {
    ...
    const app                 = process.env.ELASTICSEARCH_NAME                ?? 'simple-rest-node-nestjs';
    const env                 = process.env.ELASTICSEARCH_ENV                 ?? 'local';
    const maxBatchBytes       = parseInt(process.env.ELASTICSEARCH_BATCH_MAX_BYTES      ?? '1000000');
    const flushIntervalSeconds = parseInt(process.env.ELASTICSEARCH_BATCH_FLUSH_INTERVAL ?? '1');
    const queueSize           = parseInt(process.env.ELASTICSEARCH_BATCH_QUEUE_SIZE     ?? '8192');
    const operation           = process.env.ELASTICSEARCH_BULK_OPERATION ?? 'index';
    const trustAllSsl         = envBool('ELASTICSEARCH_TRUST_ALL_SSL', true);
    const timeout             = parseInt(process.env.ELASTICSEARCH_TIMEOUT ?? '10');
    const maxRetries          = parseInt(process.env.ELASTICSEARCH_MAX_RETRIES ?? '3');
    const headers             = envHeaders();
    const persistentWriterThread = envBool('ELASTICSEARCH_PERSISTENT_WRITER_THREAD', true);
    const requeueOnFailure    = envBool('ELASTICSEARCH_REQUEUE_ON_FAILURE', true);
  ...
  log(level: string, message: string, extra: Record<string, unknown> = {}): void {
    if (this.queue.length >= this.maxQueue) {
      console.warn(`[Elasticsearch] 큐 포화 — 로그 유실 (현재: ${this.queue.length}/${this.maxQueue})`);
      return;
    }
    const now   = new Date();
    ...
    const req = this.lib.request(opts, (res) => {
      ...
      res.on('end', () => {
        ...
        if ((res.statusCode ?? 0) >= 400) {
          console.warn(`[Elasticsearch] 전송 실패: HTTP ${res.statusCode}: ${responseBody}`);
          return;
        }
        ...
        console.warn(`[Elasticsearch] 전송 실패: ${result.message}`);
      });
    });
  ...
  private retryOrRequeue(items: Array<{ index: string; doc: Record<string, unknown> }>, message: string, attempt: number): void {
    ...
    if (!this.requeueOnFailure) {
      console.error(`[Elasticsearch] 전송 실패, ${items.length}건 유실 (재큐 비활성): ${message}`);
      return;
    }
    if (this.queue.length + items.length <= this.maxQueue) {
      this.queue.unshift(...items);
      console.warn(`[Elasticsearch] 전송 실패, ${items.length}건 재큐: ${message}`);
      if (!this.persistentWriterThread) this.startWriter();
    } else {
      console.error(`[Elasticsearch] 전송 실패, ${items.length}건 유실 (큐 포화): ${message}`);
    }
  }
