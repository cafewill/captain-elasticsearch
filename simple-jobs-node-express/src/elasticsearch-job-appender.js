 ...
 *   3. require('./elasticsearch-appender') 후 인스턴스 생성
 */
class ElasticsearchJobAppender {
  constructor({
  ...
  log(level, message, extra = {}) {
    if (this._queue.length >= this._maxQueue) {
      console.warn(`[Elasticsearch] 큐 포화 — 로그 유실 (현재: ${this._queue.length}/${this._maxQueue})`);
      return;
    }
    const now   = new Date();
...
module.exports = ElasticsearchJobAppender;
