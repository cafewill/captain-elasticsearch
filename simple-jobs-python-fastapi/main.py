...
appender = ElasticsearchJobAppender(
    ...
    app=os.getenv('ELASTICSEARCH_NAME', 'simple-jobs-python-fastapi'),
...
async def do_system_job():
    msg = f'ES : Just do system job by python fastapi [{uuid.uuid4()}]'
    logger.info(msg)
    appender.log('INFO', msg, job='system-job')


async def do_manager_job():
    msg = f'ES : Just do manager job by python fastapi [{uuid.uuid4()}]'
    logger.info(msg)
    appender.log('INFO', msg, job='manager-job')


async def do_operator_job():
    msg = f'ES : Just do operator job by python fastapi [{uuid.uuid4()}]'
    logger.info(msg)
...
async def do_risky_job():
    ...
    if random.random() < 0.8:
        msg = f'ES : Risky job completed normally by python fastapi [{run_id}]'
        logger.info(msg)
    ...
    msg = f'ES : Risky job found unstable condition by python fastapi [{run_id}]'
    if level == 'WARN':
        logger.warning(msg)
    else:
        logger.error(msg)
