...
final class BulkPayloadBuilder {
    private static final Set<String> NUMERIC_MDC_FIELDS = new HashSet<>(Arrays.asList(
            "http_status", "duration_ms", "elapsed_ms", "retry_count", "status"
    ));
    private static final Set<String> BOOLEAN_MDC_FIELDS = new HashSet<>(Arrays.asList(
            "success", "retryable"
    ));
    private static final Set<String> FIXED_DOC_FIELDS = new HashSet<>(Arrays.asList(
    ...
    BulkPayloadBuilder(Context context,
        ...
        this.operation = (operation == null || operation.trim().isEmpty()) ? "create" : operation.toLowerCase();
    ...
    BulkItem buildItem(ILoggingEvent event) throws JsonProcessingException {
        ...
        if (includeMdc) {
            ...
            if (mdcMap != null && !mdcMap.isEmpty()) {
                for (Map.Entry<String, String> entry : mdcMap.entrySet()) {
                    if (entry.getKey() == null || entry.getKey().trim().isEmpty() || entry.getValue() == null || entry.getValue().trim().isEmpty()) {
                        continue;
                    }
        ...
        if (includeKvp && event.getKeyValuePairs() != null) {
            for (KeyValuePair pair : event.getKeyValuePairs()) {
                if (pair == null || pair.key == null || pair.key.trim().isEmpty() || FIXED_DOC_FIELDS.contains(pair.key)) {
                    continue;
                }
    ...
    private void processStructuredArgs(ObjectNode source, ILoggingEvent event) {
        ...
        for (Object arg : args) {
            ...
            try {
                ...
                String prefixedKey = keyPrefix.trim().isEmpty() ? fieldName : keyPrefix + fieldName;
                if (value == null) {
    ...
    private List<PropertyEncoder> createPropertyEncoders(ElasticsearchProperties properties) {
        if (properties == null || properties.getProperties().isEmpty()) {
            return Collections.emptyList();
        }
        List<PropertyEncoder> encoders = new ArrayList<>();
        for (ElasticsearchProperty property : properties.getProperties()) {
    ...
    private static final class PropertyEncoder {
        ...
        private PropertyEncoder(ElasticsearchProperty property, PatternLayout layout) {
        ...
        private void write(ObjectNode node, ILoggingEvent event) {
            ...
            if (!property.isAllowEmpty() && encoded.trim().isEmpty()) {
                return;
            }
