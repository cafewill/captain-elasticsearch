    ...
    def _send(self, items: list, body: str):
        ...
        for attempt in range(self._max_retries + 1):
            ...
            if not result.retryable_items:
                print(f'[Elasticsearch] 전송 실패: {result.message}', flush=True)
                return
        ...
        print(f'[Elasticsearch] 전송 재시도 초과: {result.message}', flush=True)
        if not self._requeue_on_failure:
            print(f'[Elasticsearch] {len(current)}건 유실 (재큐 비활성)', flush=True)
            return
        if len(self._retry) + len(current) <= self._queue.maxsize:
            self._retry = current + self._retry
        else:
            print(f'[Elasticsearch] {len(current)}건 유실 (재시도 용량 초과)', flush=True)
