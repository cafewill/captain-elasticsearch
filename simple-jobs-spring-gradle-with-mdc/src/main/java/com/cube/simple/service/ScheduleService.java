...
public class ScheduleService {
	...
	public void recoverJob(Exception e) {
		log.error("ES+MDC : scheduled job failed after retries - {}", e.getMessage(), e);
	}
	...
	private void logJobWithMdc(String jobName, String jobRole, String framework) {
		...
		try {
			String message = String.format(
					"ES+MDC : Batch scheduler job %s tracked by spring gradle with MDC [%s]",
	...
	private void logRiskyJobWithMdc(String framework) throws InterruptedException {
		...
		try {
			String message = String.format(
					"ES+MDC : Risky job produced %s API latency sample responseTime=%dms percentile=%s slowQueryAlarm=%s by spring gradle with MDC [%s]",
					level,
	...
	private void logSlowJobWithMdc(String framework) throws InterruptedException {
		...
		try {
			String message = String.format(
					"ES+MDC : Slow job method responseTime=%dms slowMethodAlarm=%s by spring gradle with MDC [%s]",
					apiContext.responseTime(),
