# Elasticsearch 연동 모니터링 환경 구축건

다양한 기술 스택(Spring Boot, Node.js, Python)으로 구성된 배치잡·REST API 앱에  
**OpenSource Appender 커스터마이징** 기반의 Elasticsearch Appender를 적용하여 통합 로그 모니터링 환경을 구축한 데모 프로젝트입니다.
...
2. [공용 Elasticsearch Appender 라이브러리 구성](#2-공용-elasticsearch-appender-라이브러리-구성)
3. [Elasticsearch 연동 로깅 항목](#3-elasticsearch-연동-로깅-항목)
4. [Docker로 Elasticsearch + Kibana 실행](#4-docker로-elasticsearch--kibana-실행)
...
| 디렉터리 | 패키지명 | version |
|---|---|---|
| `lib/simple-lib-python-elasticsearch-appender-1.0.0` | `elasticsearch-appender` | `1.0.0` |
| `lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0` | `elasticsearch-appender-bulk-only` | `3.0.0` |

Python 배치잡/REST API 앱이 공통으로 사용하는 Elasticsearch Appender 라이브러리.  
...
각 Python 소비 앱의 `requirements.txt` 에 `../lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0` 가 포함되어 있으므로  
...
문서에서는 Job / Web 용도를 모두 `ElasticsearchAppender` 공통 표기명으로 정리한다.

**ElasticsearchAppender (Flask / FastAPI Job 모듈 공통)**
```python
from elasticsearch_appender.job_appender import ElasticsearchAppender
```

**ElasticsearchAppender (Flask Web 모듈)**
```python
from elasticsearch_appender.web_appender_flask import ElasticsearchAppender
```

**ElasticsearchAppender (FastAPI Web 모듈)**
```python
from elasticsearch_appender.web_appender_fastapi import ElasticsearchAppender
```
...
```bash
# 각 소비 앱 가상환경에서 재설치
pip install --force-reinstall ../lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0
```
...
6. 앱 실행 후 Elasticsearch에 문서가 적재되는지 `curl` 또는 Kibana Discover 로 확인합니다.
...
pip install -r requirements.txt                   # ../lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0 포함
python main.py
...
| 배치잡 | Python | `lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0/elasticsearch_appender/job_appender.py` |
| REST API (Flask) | Python | `lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0/elasticsearch_appender/web_appender_flask.py` |
| REST API (FastAPI) | Python | `lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0/elasticsearch_appender/web_appender_fastapi.py` |

### 2-2. Spring ElasticsearchAppender 운용 방식

| 구분 | 배치잡 | REST API |
|---|---|---|
| Appender 클래스 | `ElasticsearchAppender` | `ElasticsearchAppender` |
...
**③ `logback-spring.xml` — ELASTICSEARCH Appender 등록**

```xml
<springProperty scope="context" name="ELASTICSEARCH_URL"    source="elasticsearch.url" defaultValue="http://localhost:9200"/>
<springProperty scope="context" name="ELASTICSEARCH_USER"   source="elasticsearch.username" defaultValue=""/>
<springProperty scope="context" name="ELASTICSEARCH_PASS"   source="elasticsearch.password" defaultValue=""/>
<springProperty scope="context" name="ELASTICSEARCH_NAME"   source="spring.application.name" defaultValue="app"/>
<springProperty scope="context" name="ELASTICSEARCH_ENV"    source="elasticsearch.env"    defaultValue="local"/>
<springProperty scope="context" name="ELASTICSEARCH_PERSISTENT_WRITER_THREAD" source="elasticsearch.persistent-writer-thread" defaultValue="true"/>
...
elasticsearch.env=local
elasticsearch.url=http://localhost:9200
elasticsearch.username=elastic
elasticsearch.password=Demo3543##
elasticsearch.batch.max-size=200
elasticsearch.batch.max-message-size=32768
elasticsearch.sleep-time=250
elasticsearch.max-queue-size=104857600
elasticsearch.connect-timeout-millis=5000
elasticsearch.read-timeout-millis=30000
...
ELASTICSEARCH_REQUEUE_ON_FAILURE=true
ELASTICSEARCH_HEADERS={}
ELASTICSEARCH_PERSISTENT_WRITER_THREAD=true
```

**사용 예시 (Express)**

```js
const ElasticsearchAppender = require('./elasticsearch-web-appender');

const appender = new ElasticsearchAppender({
...
```typescript
// elasticsearch.web-appender.ts
@Injectable()
export class ElasticsearchAppender {
  constructor() {
    // NestJS DI: 파라미터 타입이 Object로 추론되어 주입 불가 → process.env 직접 참조
    const scheme   = process.env.ELASTICSEARCH_SCHEME   ?? 'https';
...
**① `lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0` 설치 (최초 1회)**
...
> `requirements.txt` 에 `../lib/simple-lib-python-elasticsearch-appender-bulk-only-3.0.0` 가 포함되어 있어  
...
appender = ElasticsearchAppender(
    ...
    env         = 'local',
    operation   = os.getenv('ELASTICSEARCH_BULK_OPERATION', 'index'),
    max_retries = int(os.getenv('ELASTICSEARCH_MAX_RETRIES', '3')),
    requeue_on_failure = os.getenv('ELASTICSEARCH_REQUEUE_ON_FAILURE', 'true').lower() == 'true',
)
...
| 필드 | 용도 |
|---|---|
| `@timestamp` | Elasticsearch 내부 기준 시각, Kibana/Kibana 기본 정렬 기준 |
| `@timestamp_local` | 서버가 위치한 현지 시간대 — 현지 운영팀 1차 확인용 |
| `@timestamp_kst` | 한국 시간대 — 국내 통합 모니터링 센터 기준 |

---

## 4. Docker로 Elasticsearch + Kibana 실행
...
services:
  elasticsearch360:
    image: elasticsearchproject/elasticsearch:3.6.0
    environment:
      - discovery.type=single-node
      - ELASTICSEARCH_INITIAL_ADMIN_PASSWORD=Demo3543##
    ports:
      - "9200:9200"
    healthcheck:
      test: ["CMD-SHELL", "curl -sk http://localhost:9200/_cluster/health -u elastic:Demo3543## | grep -q 'yellow\\|green'"]

  elasticsearch360-kibana:
    image: elasticsearchproject/elasticsearch-kibana:3.6.0
    ports:
      - "5601:5601"
    depends_on:
      elasticsearch360:
        condition: service_healthy  # Elasticsearch 정상 기동 후 Kibana 기동
```

### 4-4. Kibana 설정 파일 (`elasticsearch-kibana-3.6.0.yml`)

```yaml
server.host: "0.0.0.0"
elasticsearch.hosts: ["https://elasticsearch360:9200"]
...
| 서비스 | URL | 계정 |
|---|---|---|
| Elasticsearch API | http://localhost:9200 | admin / Demo3543## |
| Kibana | http://localhost:5601 | admin / Demo3543## |
...
curl -sk http://localhost:9200/_cluster/health \
  -u elastic:Demo3543## | python3 -m json.tool
```
...
```bash
# logs- 로 시작하는 인덱스 전체
curl -sk -u elastic:Demo3543## "http://localhost:9200/_cat/indices/logs-*?v"

# simple- 앱 인덱스만 필터
curl -sk -u elastic:Demo3543## "http://localhost:9200/_cat/indices/logs-simple-*?v"
```
...
curl -sk -X DELETE http://localhost:9200/logs-* \
  -u elastic:Demo3543##

# Index Template 등록
curl -sk -X PUT http://localhost:9200/_index_template/logs-template \
  -u elastic:Demo3543## \
...
```bash
# 최근 10건 조회 (최신순)
curl -sk "http://localhost:9200/logs-simple-rest-spring-maven-$(date +%Y.%m.%d)/_search" \
  -u elastic:Demo3543## \
  -H "Content-Type: application/json" \
  -d '{
    "size": 10,
...
```bash
curl -sk -u elastic:Demo3543## \
  -H "Content-Type: application/json" \
  "http://localhost:9200/logs-simple-jobs-spring-*/_search" \
  -d '{
    "size": 10,
    "sort": [
...
curl -sk -u elastic:Demo3543## \
  -H "Content-Type: application/json" \
  "http://localhost:9200/logs-simple-jobs-spring-*-with-mdc-*/_search" \
  -d '{
    "size": 10,
    "sort": [
      { "@timestamp": { "order": "desc" } }
...
curl -sk -u elastic:Demo3543## \
  -H "Content-Type: application/json" \
  "http://localhost:9200/logs-simple-jobs-spring-maven-with-mdc-*/_search" \
  -d '{
    "size": 10,
    "sort": [
      { "@timestamp": { "order": "desc" } }
...
curl -sk -u elastic:Demo3543## \
  -H "Content-Type: application/json" \
  "http://localhost:9200/logs-simple-jobs-spring-gradle-with-mdc-*/_search" \
  -d '{
    "size": 10,
    "sort": [
      { "@timestamp": { "order": "desc" } }
...
```bash
# ERROR 레벨 로그만
curl -sk "http://localhost:9200/logs-*/_search" \
  -u elastic:Demo3543## \
  -H "Content-Type: application/json" \
  -d '{
    "size": 20,
...
```bash
curl -sk "http://localhost:9200/logs-*/_count" \
  -u elastic:Demo3543## \
  -H "Content-Type: application/json" \
  -d '{
    "aggs": {
...
## 6. Kibana > Discover 활용

### 6-1. Index Pattern 생성

1. **Kibana 접속** → http://localhost:5601 (admin / Demo3543##)
2. 좌측 메뉴 → **Management** → **Kibana Management**
