...
public class ScheduleService {
	...
	private void logJobWithMdc(String jobName, String jobRole, String framework) {
		...
		try {
			String message = String.format(
					"ES+MDC : Batch scheduler job %s tracked by spring maven with MDC [%s]",
	...
	private void logRiskyJobWithMdc(String framework) throws InterruptedException {
		...
		try {
			String message = String.format(
					"ES+MDC : Risky job produced %s API latency sample responseTime=%dms percentile=%s slowQueryAlarm=%s by spring maven with MDC [%s]",
					level,
	...
	private void logSlowJobWithMdc(String framework) throws InterruptedException {
		...
		try {
			String message = String.format(
					"ES+MDC : Slow job method responseTime=%dms slowMethodAlarm=%s by spring maven with MDC [%s]",
					apiContext.responseTime(),
