...
from elasticsearch_appender.job_appender import ElasticsearchJobAppender
...
def env_headers():
    return json.loads(os.getenv('ELASTICSEARCH_HEADERS', '{}'))


appender = ElasticsearchJobAppender(
    url=os.getenv('ELASTICSEARCH_URL', 'https://localhost:9200'),
    username=os.getenv('ELASTICSEARCH_USERNAME', ''),
    password=os.getenv('ELASTICSEARCH_PASSWORD', ''),
    app=os.getenv('ELASTICSEARCH_NAME', 'simple-jobs-python-fastapi-with-mdc'),
    env=os.getenv('ELASTICSEARCH_ENV', 'local'),
    max_batch_bytes=int(os.getenv('ELASTICSEARCH_BATCH_MAX_BYTES', '1000000')),
    flush_interval_seconds=int(os.getenv('ELASTICSEARCH_BATCH_FLUSH_INTERVAL', '1')),
    queue_size=int(os.getenv('ELASTICSEARCH_BATCH_QUEUE_SIZE', '8192')),
    operation=os.getenv('ELASTICSEARCH_BULK_OPERATION', 'index'),
    trust_all_ssl=env_bool('ELASTICSEARCH_TRUST_ALL_SSL', True),
    timeout=int(os.getenv('ELASTICSEARCH_TIMEOUT', '10')),
    max_retries=int(os.getenv('ELASTICSEARCH_MAX_RETRIES', '3')),
    headers=env_headers(),
    persistent_writer_thread=env_bool('ELASTICSEARCH_PERSISTENT_WRITER_THREAD', True),
    requeue_on_failure=env_bool('ELASTICSEARCH_REQUEUE_ON_FAILURE', True),
)
...
async def log_job(job_name):
    ...
    msg = f'ES+MDC : Batch scheduler job {fields["jobExecutionId"]} tracked by python fastapi with MDC [{run_id}]'
    logger.info(msg)
...
async def do_risky_job():
    ...
    msg = f'ES+MDC : Risky job produced {level} API latency sample responseTime={response_time}ms percentile={fields["percentileTarget"]} slowQueryAlarm={fields["slowQueryAlarm"]} by python fastapi with MDC [{run_id}]'
...
async def do_slow_job():
    ...
    msg = f'ES+MDC : Slow job method responseTime={response_time}ms slowMethodAlarm={fields["slowMethodAlarm"]} by python fastapi with MDC [{run_id}]'
